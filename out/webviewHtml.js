...
exports.buildPlayerHtml = buildPlayerHtml;
const utils_1 = require("./utils");
function buildPlayerHtml(scrim) {
...
<div class="header">
  ...
  <div class="header-title" title="${(0, utils_1.escapeHtml)(scrim.title)}">${(0, utils_1.escapeHtml)(scrim.title)}</div>
...
// ── HTML sub-builders ──────────────────────────────────────────────────────
function buildVideoBlock(scrim) {
    ...
    if (!url) {
      ...
      <div style="display:flex; flex-direction:row; align-items:center; width: 100%; gap: 12px;">
        ...
        <span id="customTimeDisplay" style="font-family: monospace; font-size: 13px; color: var(--muted); min-width: 80px; text-align: center;">
          0:00 / ${(0, utils_1.formatTime)(maxTime)}
        </span>
    ...
    <audio id="localVideo" controls preload="metadata" style="width: 100%; max-width: 600px; outline: none;">
      <source src="${(0, utils_1.escapeHtml)(url)}">
    </audio>
  </div>`;
}
function buildChaptersHtml(chapters) {
    ...
    return chapters
        .map(ch => `<div class="chapter-item"
        onclick="jumpToChapter(${ch.timestamp})">
        <div class="dot"></div>
        <span class="ch-name">${(0, utils_1.escapeHtml)(ch.title)}</span>
        <span class="ch-time">${(0, utils_1.formatTime)(ch.timestamp)}</span>
      </div>`)
        .join('\n');
}
